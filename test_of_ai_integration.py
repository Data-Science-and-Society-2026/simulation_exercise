...
from stt_tts import tts_functions
import re
import threading
...
pdf_folder = os.getenv('PDF_FOLDER', r"pdf")
...
tts_functions.text_to_speech(f"Hello, How can I call you?")
user_response = tts_functions.speech_to_text()
student_name = get_student_name(user_response)

tts_functions.text_to_speech(f"Nice to meet you, {student_name}! What course would you like me to teach?")
course_input = user_response = tts_functions.speech_to_text()
course_name = get_course_name(course_input)

tts_functions.text_to_speech(f"How familiar are you with {course_name}? (Beginner, Intermediate, Advanced): ")
level_input = user_response = tts_functions.speech_to_text()
familiarity = get_familiarity_level(level_input)

tts_functions.text_to_speech(f"Great! I will be your tutor for {course_name} at the {familiarity} level. Let's begin!")
...
while True:
    print("Ask your AI tutor a question (or type 'exit' to quit): ")
    user_input = tts_functions.speech_to_text()
    
    if user_input.lower().strip(" .") in ['exit', 'quit', 'bye']:
        tts_functions.text_to_speech("Goodbye!")
        print("Goodbye! 👋")
        break
    ...
    tts_lock = threading.Lock()  # Prevents overlapping TTS calls
    ...
    for chunk in llm.stream(final_prompt):
        ...
        for sentence in sentences[:-1]:  # Process all full sentences
            if sentence.strip():  # Avoid empty or single-word outputs
                with tts_lock:  # Ensure only one sentence at a time
                    tts_functions.text_to_speech(sentence.strip())
                print(sentence.strip(), flush=True)
                full_response += sentence.strip() + " "  # Append to the full response
    ...
    if buffer.strip():
        with tts_lock:
            tts_functions.text_to_speech(buffer.strip())
